...
require 'support/mathcers/have_text'
...
app.start_server do
  spec_types.each do |type, (path, controller)|
    ...
    decribe "in a #{type}" do
      it "runs in the correct environment" do
        expect(page).to have_text(app.evironment).in("#environment")
      end
      ...
      it "can use build-in helpers" do
        expect(page).to have_text("Once upon a...").in("#truncated")
      end

      it "can use build-in private helpers" do
      ...
      it "can be passed to url helpers" do
        expect(page).to have_text("http://example.com:12345/en/posts/1").in("url_with_decorator")
      end
